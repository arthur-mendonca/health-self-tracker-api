...
### Fase 3: Setup e Construção do Front-end (Pasta frontend-app)

Saia da pasta do backend. Crie uma nova pasta independente chamada frontend-app.
Inicialize um projeto SvelteKit do zero dentro de frontend-app.
Para rodar o front-end via Deno, configure a task apropriada no projeto SvelteKit: deno task dev (usando o Vite).
Instale e configure Tailwind CSS.
Construa o formulário principal. Garanta que o campo de Métricas permita adicionar pares "Chave: Valor" dinâmicos. Crie os selects inteligentes com autocompletar e criação on-the-fly.
